...
def get_gene_mch(species, gene, outliers):
    ...
    if len(dataframe_list) == 0:
        ...
        print(type(dataframe_list[0]))
        result = []
        for list in dataframe_list:
            result += list
        return result
...
@cache.memoize(timeout=3600)
def get_mch_heatmap(species, level, ptile_start, ptile_end, query):
    ...
    for gene in query:
        gene_info[gene[0]] = mean_cluster_mch(get_gene_mch(species, gene[1], True), level)

    x, y, text, mch = list(), list(), list(), list()

    for cluster_number in list(gene_info[list(gene_info.keys())[0]].keys()):
...
def mean_cluster_mch(gene_info, level):
    """Gets average mch level of a gene for each cluster

        Arguments:
            gene_info (list): dict with mCH data for each sample. Keys are samp, tsne_x, tsne_y, cluster_label, cluster_ordered,
                original, normalized.
            level (str): Type of mCH data. Should be "original" or "normalized".
...
def get_metadata(ensemble):
    ...
    meta_path = "{}/ensembles/{}/metadata.csv".format(
